...
namespace Net.UI
{
    ...
    public class WidgetPage : ContentPage
    {
        ...
        {
            if (Content is Xamarin.Forms.VisualElement ve)
            {
                return ve
                    .GetAllChildren()
                    .Where(x => x is Widget)
        ...
        protected override void OnAppearing()
        {
            ...
            var mvm = BindingContext as XViewModel;
        ...
        protected override void OnDisappearing()
        {
            ...
            {
                isStarted = false;
                if (BindingContext is XViewModel mvm)
                    mvm.OnStop();
                lastContext = null;
            }
        ...
        protected override void OnBindingContextChanged()
        {
            ...
            {
                if (lastVm is XViewModel mvm)
                    mvm.OnUnbind(this);
            }

            if (BindingContext is ViewModel vm)
            {
                if (vm is XViewModel mvm)
                    mvm.OnBind(this);
                if (!isStarted)
                {
                    if (lastVm is XViewModel lastMvm)
                        lastMvm?.OnStop();
                    isStarted = true;
                    vm.OnStart();
                    lastContext = new WeakReference<ViewModel>(vm);
